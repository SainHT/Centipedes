...
if (downloadRaylib) then
    ...
    project (workspaceName)
        ...
        vpaths 
        {
            ...
            ["Source Files/*"] = {"../src/**.c", "src/**.cpp", "../src/**.s"},
            ["Assembly Files/*"] = {"../src/**.s"},
            ["Windows Resource Files/*"] = {"../src/**.rc", "src/**.ico"},
        }
        
        files {"../src/**.c", "../src/**.cpp", "../src/**.s", "../src/**.h", "../src/**.hpp", "../include/**.h", "../include/**.hpp"}
        ...
        -- Assembly file compilation rules
        filter "files:**.s"
            buildmessage "Assembling %{file.relpath}"
            buildcommands {
                "as --64 -o \"%{cfg.objdir}/%{file.basename}.o\" \"%{file.relpath}\""
            }
